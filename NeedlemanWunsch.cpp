...
void NeedlemanWunsch::Analyze()
{
    total_length = static_cast<int>(aligned_a.length());
    matches = static_cast<int>(count(match_symbols.begin(), match_symbols.end(), '|'));
    gaps = static_cast<int>(count(aligned_a.begin(), aligned_a.end(), '-') + count(aligned_b.begin(), aligned_b.end(), '-'));
    alignment_score = score_matrix.back().back();
    match_percentage = matches / total_length;
    gap_percentage = gaps / total_length;
}
...
void NeedlemanWunsch::PrintResults() const 
{
   ...
   cout << "Alignment Score: " << alignment_score << "\n"
        << "Match Percentage: " << 100.0 * match_percentage  << "%\n"
        << "Gap Percentage: " << 100.0 * gap_percentage  << "%\n";
}